import type { BrandedUUID, RepositoryStrategyVideoThumbnail, TypeormPagination } from "../../types";
import { getBrandedUUID } from "../../utils/brand";
import { AppDataSource } from "../datasource";
import * as Entities from "../models";
import { ThumbnailRepository } from "./thumbnail";

export class VideoThumbnailRepository implements RepositoryStrategyVideoThumbnail {
  repository;
  repositories;

  constructor() {
    this.repository = AppDataSource.getRepository(Entities.VideoThumbnailTypeorm);
    this.repositories = {
  ...
  async paginateItems$({ limit, page }: TypeormPagination): Promise<{
    result: Entities.VideoThumbnailTypeorm[];
    count: number;
  }> {
    const pagination = {
      limit: 100,
  ...
  async listItems$(): Promise<Entities.VideoThumbnailTypeorm[]> {
    const list = await this.repository.find({});

    return list;
  }

  async getItem$(payload: BrandedUUID): Promise<Entities.VideoThumbnailTypeorm | null> {
  ...
  async createItem$(payload: Entities.VideoThumbnailTypeorm): Promise<Entities.VideoThumbnailTypeorm> {
    const { uuid } = payload;
  ...
  async updateItem$(payload: Entities.VideoThumbnailTypeorm): Promise<Entities.VideoThumbnailTypeorm> {
